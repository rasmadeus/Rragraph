...
void QwtPlot::setAbscissMin(double value){
    AxisData &d = *d_axisData[xBottom];
    d.doAutoScale = false;
    d.scaleDiv.invalidate();
    d.minValue = value;
    autoRefresh();
...
void QwtPlot::setAbscissMax(double value){
    AxisData &d = *d_axisData[xBottom];
    d.doAutoScale = false;
    d.scaleDiv.invalidate();
    d.maxValue = value;
...
void QwtPlot::setAbscissStep(double value){
    AxisData &d = *d_axisData[xBottom];
    d.doAutoScale = false;
    d.scaleDiv.invalidate();
    d.stepSize = value;
...
void QwtPlot::setOrdinateMin(double value){
    AxisData &d = *d_axisData[yLeft];
    d.doAutoScale = false;
    d.scaleDiv.invalidate();
    d.minValue = value;
    autoRefresh();
...
void QwtPlot::setOrdinateMax(double value){
    AxisData &d = *d_axisData[yLeft];
    d.doAutoScale = false;
    d.scaleDiv.invalidate();
    d.maxValue = value;
...
void QwtPlot::setOrdinateStep(double value){
    AxisData &d = *d_axisData[yLeft];
    d.doAutoScale = false;
    d.scaleDiv.invalidate();
    d.stepSize = value;
...
void QwtPlot::createAxesIntervalChangedSignal(){
    emit axesIntervalChanged(
        axisInterval(xBottom).minValue(), axisInterval(xBottom).maxValue(), getStep(xBottom),
        axisInterval(yLeft).minValue(),   axisInterval(yLeft).maxValue(),   getStep(yLeft)
    );
}

double QwtPlot::getStep(Axis axis){
    return axisScaleDiv(axis)->ticks(QwtScaleDiv::MajorTick).size() > 1
           ?  axisScaleDiv(axis)->ticks(QwtScaleDiv::MajorTick)[1] - axisScaleDiv(axis)->ticks(QwtScaleDiv::MajorTick)[0]
           :  0;
}

QString QwtPlot::getNameLegendPosition(int i)
{
    switch(i){
        case 0 : return tr("Left");
        case 1 : return tr("Right");
        case 2 : return tr("Bottom");
        case 3 : return tr("Top");
    }
    return tr("Don't show the legend");
}

QIcon QwtPlot::getIconLegendPosition(int i)
{
    switch(i){
        case 0 : return QIcon(":/resources/Styles/LegendPosition/left.png");
        case 1 : return QIcon(":/resources/Styles/LegendPosition/right.png");
        case 2 : return QIcon(":/resources/Styles/LegendPosition/bottom.png");
        case 3 : return QIcon(":/resources/Styles/LegendPosition/top.png");
    }
    return QIcon(":/resources/Styles/LegendPosition/nothing.png");
}
